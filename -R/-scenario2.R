...
simulate_data <- function(n_sim = 2e2) {
  ...
  D <- D +
    ...
    # node("W", distr = "runif", min = 0, max = 1.9) +
    node("W", distr = "runif", min = 0, max = 1) +
    node("A", distr = "rbinom", size = 1, prob = .15 + .5*as.numeric(W > .75)) +
    node("Trexp", distr = "rexp", rate = 1 + .3*(W)^2 - .8*A) +
    node("Cweib", distr = "rweibull", shape = 1 - .5*W, scale = 75) +
    node("T", distr = "rconst", const = round(Trexp*1,0)) +
    node("C", distr = "rconst", const = round(Cweib*1, 0)) +
  ...
  true_surv_one <- function(q, W, A = 1) sapply(W, function(w) {1 - pexp(q, rate = 1 + .3*w^2 - .8*A)})
