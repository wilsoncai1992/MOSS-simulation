...
simulate_data <- function(n_sim = 2e2) {
  ...
  D <- D +
    ...
    node("Trexp", distr = "rlnorm", meanlog = 2 - 2*W + 1*A) +
    ...
    # node("T", distr = "rconst", const = ceiling(Trexp*.2)+1) +
  ...
  true_surv_one <- function(q, W, A = 1) sapply(W, function(w) {1 - plnorm(q, meanlog = 2 - 2*w + 1*A)})
